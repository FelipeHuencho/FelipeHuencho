# Bienvenid@ a mi perfil

[![Portfolio](https://img.shields.io/badge/-Portafolio-red?style=flat&logo=appveyor&logoColor=white)](https://portal.hipona.cl/inicioSesion/login/?next=/)
[![GitHub](https://img.shields.io/badge/-GitHub-000?style=flat&logo=github&logoColor=white)](https://github.com/FelipeHuencho)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-blue?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/felipe-huencho-48ab20370/)
[![Gmail](https://img.shields.io/badge/-Gmail-c14438?style=flat-square&logo=gmail&logoColor=white)](mailto:fe.huencho@gmail.com)

<img align="right" alt="GIF" src="https://github.com/abhisheknaiidu/abhisheknaiidu/blob/master/code.gif?raw=true" width="45%" />

<p width="45%">
Hola soy Felipe Huencho, Te cuento brevemente sobre mí.
  <ul>
    <li>Soy <b>ingeniero en informática</b></li>
    <li>Enfocado en <b>desarrollo web y backend</b></li>
    <li>Trabajo con <b> Django y bases de datos relacionales</b></li>
    <li>Desarrollo <b>páginas web informativas</b> , <b>Aplicaciónes web </b> </li>
    <li>Puedo ayudarte con <b>APIs REST, CRUD, deploy y configuración de hosting</b></li>
    <li>Contacto directo: <b>fe.huencho@gmail.com</b></li>
  </ul>

Me enfoco en crear soluciones claras y funcionales para negocios que necesitan presencia online o sistemas simples y mantenibles.
<br>
Trabajo con código limpio, buenas prácticas y comunicación directa con el cliente.
<br>
Si quieres conversar sobre un proyecto o una idea, puedes escribirme sin problema.
</p>

## Mi stack Tecnológico

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,django,html,css,js,postgresql,git,github,linux,nginx,mysql,postman" />
</p>

## Estadísticas de GitHub 

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=FelipeHuencho&show_icons=true&theme=radical&hide_border=true" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=FelipeHuencho&layout=compact&theme=radical&hide_border=true" width="40%" />
</p>

## Proyectos destacados

### Intranet Web – Proyecto Académico / Aplicación Web

Desarrollo de una **intranet web** orientada a la gestión interna, enfocada en organización de información y operaciones básicas de usuarios.

**Características principales:**
- Autenticación de usuarios
- Operaciones CRUD
- Integración con base de datos relacional
- Backend estructurado y mantenible
- Enfoque en uso interno (intranet)

**Tecnologías utilizadas:**
- Backend con Django
- Base de datos relacional
- HTML, CSS y JavaScript
- Control de versiones con Git y GitHub

Repositorio:  
https://github.com/Date2099/capstone_803D_Equipo3_KevinCartagen_WillyCastro_FelipeHuencho



## Puedes contactarte conmigo mediante

Email: fe.huencho@gmail.com  
LinkedIn: https://www.linkedin.com/in/felipe-huencho-48ab20370/  
GitHub: https://github.com/FelipeHuencho
